...
<body>
    <div id="info">
      ...
      <span class="title"><input id="elevationIdentifier" type="text" value=9 /><input id="elevationSlider" type="range" max=100 min=-100 value=0 onchange="changeElevation()"/></span>
    </div>
    ...
    function changeElevation(event) {
        var targetChild = document.getElementById("elevationIdentifier").value;
        console.log("Searching for", targetChild);
        for (let i in scene.children) {
            if (scene.children[i].id.toString() === targetChild) {
                console.log("found", scene.children[i].modelName, "to add", document.getElementById("elevationSlider").value);
            }
        }
    }
    </script>
